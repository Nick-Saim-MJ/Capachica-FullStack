...
export class MisReservasService {
  ...
  // Imprimir reserva (generar PDF)
  imprimirReserva(id: number): Observable<Blob> {
    return this.http.get(`${this.API_URL}/reservas/${id}/imprimir`, {
      headers: this.getHeaders(),
      responseType: 'blob'
    });
  }
  ...
  // Pagar reserva
  pagarReserva(id: number, datosPago: PagarReservaRequest): Observable<PagarReservaResponse> {
    return this.http.post<PagarReservaResponse>(`${this.API_URL}/reservas/${id}/pagar`, datosPago, {
      headers: this.getHeaders()
    });
  }
...
export interface PagarReservaRequest {
  metodo_pago: 'yape' | 'visa' | 'mastercard' | 'efectivo';
  monto: number;
  referencia_pago?: string;
  notas_pago?: string;
}

export interface PagarReservaResponse {
  success: boolean;
  message: string;
  data?: {
    reserva: Reserva;
