...
export class CarritoComponent implements OnInit {
  ...
  confirmarReserva() {
    ...
    // Paso 1: Confirmar la reserva (convertir carrito en reserva pendiente)
    this.carritoService.confirmarReserva().subscribe({
      next: (response) => {
        if (response.success && response.data) {
          const reservaCreada = response.data;
          this._successMessage.set('Reserva creada. Procesando pago...');

          // Calcular monto total
          const montoTotal = reservaCreada.servicios?.reduce((total: number, servicio: any) => {
            return total + (servicio.precio || 0) * (servicio.cantidad || 1);
          }, 0) || 0;

          if (montoTotal > 0) {
            // Paso 2: Abrir Culqi para procesar el pago
            const email = reservaCreada.usuario?.email || 'cliente@turismocapachica.com';
            const descripcion = `Reserva ${reservaCreada.codigo_reserva} - ${reservaCreada.servicios?.length || 0} servicio(s)`;
            ...
            this.culqiService.realizarPagoCompleto(
              ...
              next: (pagoResponse) => {
                if (pagoResponse.success) {
                  this._successMessage.set('¡Pago procesado exitosamente! Redirigiendo...');
                  setTimeout(() => {
                    window.location.href = '/admin/reservas/mis-reservas';
                  }, 2000);
                } else {
                  this._errorMessage.set('La reserva se creó pero hubo un error en el pago. Puedes pagar desde "Mis Reservas".');
                  setTimeout(() => {
                    window.location.href = '/admin/reservas/mis-reservas';
                  }, 3000);
                }
              ...
              error: (pagoError) => {
                ...
                this._errorMessage.set('La reserva se creó pero el pago fue cancelado. Puedes pagar desde "Mis Reservas".');
                setTimeout(() => {
                  window.location.href = '/admin/reservas/mis-reservas';
                }, 3000);
                this._isConfirming.set(false);
