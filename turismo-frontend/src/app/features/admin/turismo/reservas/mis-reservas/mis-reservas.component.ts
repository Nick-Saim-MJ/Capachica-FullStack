...
import { CulqiService } from '../../../../../core/services/culqi.service'; // NUEVO
        ...
        <div *ngFor="let reserva of reservasFiltradas" 
          ...
          <div *ngIf="reservaExpandida === reserva.id" class="px-6 py-4">
            ...
            <div class="mt-6 flex flex-wrap gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
              ...
              <!-- Nuevos botones de imprimir y pagar -->
              <button (click)="imprimirReserva(reserva.id!)" 
...
export class MisReservasComponent implements OnInit {
  ...
  private culqiService = inject(CulqiService); // NUEVO
  ...
  imprimirReserva(reservaId: number): void {
    this.misReservasService.imprimirReserva(reservaId).subscribe({
      next: (blob: Blob) => {
        // Crear URL del blob
        ...
        link.href = url;
        link.download = `reserva_${reservaId}.pdf`;
      ...
      error: (error) => {
        ...
        alert('Error al generar el PDF de la reserva');
      }
    });
  }

  // Método para pagar reserva CON CULQI
  pagarReserva(reserva: Reserva): void {
    if (!reserva.servicios || reserva.servicios.length === 0) {
      alert('No hay servicios en esta reserva para pagar');
      return;
    }

    // Calcular monto total
    const montoTotal = reserva.servicios.reduce((total, servicio) => {
      return total + (servicio.precio || 0) * (servicio.cantidad || 1);
    }, 0);

    if (montoTotal <= 0) {
      alert('El monto total de la reserva es 0');
      return;
    }

    // Obtener email del usuario (puedes obtenerlo del servicio de autenticación)
    const email = reserva.usuario?.email || 'cliente@turismocapachica.com';
    const descripcion = `Reserva ${reserva.codigo_reserva} - ${reserva.servicios.length} servicio(s)`;

    this.procesandoPago = true;

    // Usar Culqi para procesar el pago
    this.culqiService.realizarPagoCompleto(
      reserva.id!,
      montoTotal,
      email,
      descripcion,
      {
        nombreEmpresa: 'Turismo Capachica',
        cuotas: 0 // Sin cuotas por defecto, puedes cambiar esto
      }
    ).subscribe({
      next: (response) => {
        if (response.success) {
          alert(`¡Pago procesado exitosamente!\n\nID de cargo: ${response.data?.pago.charge_id}\nMonto: S/. ${response.data?.pago.monto.toFixed(2)}`);
          this.cargarReservas(); // Recargar reservas para ver el cambio de estado
        } else {
      ...
      error: (error) => {
        console.error('Error al procesar pago con Culqi:', error);
        const mensajeError = error?.message || error?.error?.message || 'Error desconocido al procesar el pago';
  ...
  // Método anterior para pagar reserva (puedes mantenerlo como fallback)
  pagarReservaManual(reserva: Reserva): void {
    if (!reserva.servicios || reserva.servicios.length === 0) {
      alert('No hay servicios en esta reserva para pagar');
      return;
    }
    ...
    // Mostrar modal de pago con datos específicos
    this.reservaParaPago = reserva;
    this.mostrarModalPago = true;
    this.metodoPagoSeleccionado = '';
  }

  // Método mejorado para procesar pago
  procesarPago(): void {
    if (!this.reservaParaPago || !this.metodoPagoSeleccionado) {
      return;
    }
    ...
    // Calcular monto total
    const montoTotal = this.reservaParaPago.servicios?.reduce((total, servicio) => {
      return total + (servicio.precio || 0) * (servicio.cantidad || 1);
    }, 0) || 0;

    const datosPago = {
      metodo_pago: this.metodoPagoSeleccionado as 'yape' | 'visa' | 'mastercard' | 'efectivo',
      monto: montoTotal,
      referencia_pago: '', // Se puede agregar un campo para esto
      notas_pago: ''
    };

    this.misReservasService.pagarReserva(this.reservaParaPago.id!, datosPago).subscribe({
      next: (response) => {
        if (response.success) {
          alert('Pago procesado exitosamente');
