...
use App\Http\Controllers\API\Payment\CulqiController; // NUEVO - Pagos con Culqi
...
// ===== CULQI - CONFIGURACIÓN PÚBLICA =====
Route::get('/pagos/culqi/config', [CulqiController::class, 'getConfig']);
...
Route::middleware('auth:sanctum')->group(function () {
    ...
    Route::prefix('reservas')->group(function () {
        ...
        // Nuevas funcionalidades (DEBEN ir ANTES de las rutas REST)
        Route::get('{id}/imprimir', [ReservaController::class, 'imprimirReserva']);
        Route::post('{id}/pagar', [ReservaController::class, 'pagarReserva']);

        // Rutas REST convencionales (deben ir AL FINAL)
    ...
    // ===== PAGOS CON CULQI =====
    Route::prefix('pagos/culqi')->group(function () {
        Route::post('/procesar', [CulqiController::class, 'procesarPago']); // Procesar pago
        Route::post('/webhook', [CulqiController::class, 'webhook'])->withoutMiddleware(['auth:sanctum']); // Webhook de Culqi (sin autenticación)
    });
